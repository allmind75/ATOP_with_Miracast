...
public class KeyboardMouseActivity extends Activity implements OnClickListener {
	...
	private boolean is_Socket_connect = true; // 소켓 연결 확인

	// Secondary Display

	private DisplayManager mDisplayManager;
	private final SparseArray<RemotePresentation> mActivePresentations = new SparseArray<RemotePresentation>();
	private final SparseArray<VideoPresentation> mActiveVedioPresentations = new SparseArray<VideoPresentation>();
	...
	private VideoPresentation videoPresentation = null;
	...
	static Handler StateHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (flag == 1) {
				...
				tv_cur_state.setText("잠시만 기다려주세요...");
			}
		}
	};
	...
	public void video_exit_dialog() {
		...
		dialg.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				media_seekbar.setSecondaryProgress(0);
				tv_cur_time.setText(" ");
	...
	private void showVedioPresentaion() {
		if (presentationDisplays.length > 0) {
			Display display = presentationDisplays[0];
			videoPresentation = new VideoPresentation(this, display,
					mVideoViewMain);
	...
	private void hideVedioPresentation() {
		if (presentationDisplays.length > 0) {
			Display display = presentationDisplays[0];
			final int displayId = display.getDisplayId();
			VideoPresentation vedioPresentation = mActiveVedioPresentations
					.get(displayId);
	...
	private final class VideoPresentation extends Presentation {
		...
		protected void onCreate(Bundle savedInstanceState) {
			...
			View viewControl = controlInflater
					.inflate(R.layout.videoview, null);
			LayoutParams layoutParamsControl = new LayoutParams(
					LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
